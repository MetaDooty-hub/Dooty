...
function FeedbackButton({ gamertag }) {
  ...
  const TYPES = [
    ...
    { id: 'weapons', label: 'Weapons/Attachments' },
  ];
  async function submit() {
    if (!message.trim()) return;
    setLoading(true);
    await sbFetch('feedback', { method: 'POST', body: JSON.stringify({ gamertag: gamertag || 'Anonymous', rating: rating || null, type, message: message.trim() }) });
    setLoading(false); setSuccess(true);
    setTimeout(() => { setSuccess(false); setOpen(false); setRating(0); setMessage(''); setType('general'); }, 2000);
  }
  return (<>
    <button onClick={() => setOpen(o => !o)} style={{ position: 'fixed', bottom: '24px', right: '24px', width: '52px', height: '52px', borderRadius: '50%', background: open ? '#ff8c00' : '#00e5ff', border: 'none', cursor: 'pointer', fontSize: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9000, transition: 'all 0.2s' }} title="Send Feedback">{open ? '✕' : '💬'}</button>
...
```
584      </main>
585          <FeedbackButton gamertag={gamertag} />
586      </div>
587    </>);
588  }
    </div>
